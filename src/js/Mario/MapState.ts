...
const worldMap = Enjine.Resources.Images?.worldMap;
...
export class MapState extends Enjine.GameState {
	...
	Enter() {
		...
		this.WaterSprite.Image = worldMap;
		...
		this.DecoSprite.Image = worldMap;
		...
		this.HelpSprite.Image = worldMap;
		...
		this.SmallMario.Image = worldMap;
		...
		this.LargeMario.Image = worldMap;
	...
	GenerateLevel() {
		...
		for (let i = 0; i < 21; i++) {
			...
			this.Data[i] = [];
			for (let b = 0; b < 16; b++) {
				let c = new ImprovedNoise().PerlinNoise(i * 10 + h, b * 10 + i);
				let e = new ImprovedNoise().PerlinNoise(i * 10 + j, b * 10 + k);
				c -= e;
				c *= 2;
		...
		for (let x = 0; x < 21; x++)
			for (let y = 0; y < 16; y++)
				if (this.Level[x][y] === MapTile.Grass && (x !== this.XFarthestCap || y !== this.YFarthestCap - 1)) {
					let c = new ImprovedNoise().PerlinNoise(x * 10 + h, y * 10 + i);
					if (c > 0)
						this.Level[x][y] = MapTile.Decoration;
				}
		return !0;
	}
	FindConnection(width: number, height: number) {
	...
	Connect(X: number, Y: number, width: number, height: number) {
		let d = 1E4, x1 = 0, y1 = 0;
		for (let x = 0; x < width; x++)
			for (let y = 0; y < height; y++)
	...
	DrawRoad(x1: number, y1: number, x2: number, y2: number) {
		let d = !1;
		Math.random() > .5 && (d = !0);
		if (d) {
			while (x1 > x2) {
	...
	FindCaps(width: number, height: number) {
		let d = -1, f = -1;
		for (let x = 0; x < width; x++)
			for (let y = 0; y < height; y++)
	...
	Travel(x: number, y: number, c: number, e: number) {
	...
	Update(sprite: number) {
		if (this.WorldNumber !== 8) {
			...
			let MARIO = this[mario as "LargeMario" | "SmallMario"] as Enjine.AnimatedSprite;
			MARIO.X = this.XMario + this.XMarioA * sprite | 0;
			MARIO.Y = this.YMario + (this.YMarioA * sprite | 0) - (Mario.MarioCharacter.Large ? 22 : 6);
			MARIO.Update(sprite);
		}
	}
	TryWalking(moveX: number, moveY: number) {
	...
	CalcDistance(x: number, y: number, moveX: number, moveY: number) {
		for (let d = 0; true; d++) {
			x += moveX;
			y += moveY;
			if (this.Level[x][y] !== MapTile.Road ||
					...
					switch (this.Level[b][c]) {
						case Water:
							if (this.IsWater(b * 2 - 1, c * 2 - 1))
								setSprite(this.WaterSprite as Enjine.AnimatedSprite, -8, -8, this.camera);
							break;
						case Decoration:
							setSprite(this.DecoSprite as Enjine.AnimatedSprite, 0, 0, this.camera);
							break;
						case Level:
							if (this.Data[b][c] === -2)
								setSprite(this.HelpSprite as Enjine.AnimatedSprite, 16, -16, this.camera);
							break;
					}
					function setSprite(sprite: Enjine.AnimatedSprite, n1: number, n2: number, camera: Enjine.Camera) {
						sprite.X = b * 16 + n1;
						sprite.Y = c * 16 + n2;
	...
	CheckForChange(ctx: Enjine.GameStateContext) {
		if (this.WorldNumber === 8)
			ctx.ChangeState(new WinState());
		if (this.EnterLevel)
			ctx.ChangeState(new LevelState(this.LevelDifficulty, this.LevelType));
	}
}
