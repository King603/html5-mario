...
export class Character extends NotchSprite {
    ...
    Blink(a) {
        ...
        this.Image = this.Large
            ...
            : Enjine.Resources.Images?.smallMario;
    ...
    Move() {
        if (this.WinTime > 0) {
            ...
            this.CalcPic();
            if (this.Sliding) {
                this.World.AddSprite(new Sparkle(this.World, (this.X + Math.random() * 4 - 2 | 0) + this.Facing * 8, (this.Y + Math.random() * 4 | 0) - 24, Math.random() * 2 - 1, Math.random()));
                this.Ya *= .5;
            }
            this.OnGround = !1;
    ...
    SubMove(a, b) {
        ...
        let x = this.X + a;
        let y = this.Y + b;
        if (b > 0 && (this.IsBlocking(x - this.Width, y, a, 0) ||
            this.IsBlocking(x + this.Width, y, a, 0) ||
            this.IsBlocking(x - this.Width, y + 1, a, b) ||
            this.IsBlocking(x + this.Width, y + 1, a, b)))
            c = !0;
        if (b < 0 && (this.IsBlocking(x, y - this.Height, a, b) || c ||
    ...
    IsBlocking(a, b, c, e) {
        ...
        if ((Tile.Behaviors[d & 255] & Tile.PickUpable) > 0) {
            ...
            for (let d = 0; d < 2; d++)
                for (let f = 0; f < 2; f++)
                    this.World.AddSprite(new Sparkle(this.World, a * 16 + d * 8 + (Math.random() * 8 | 0), b * 16 + f * 8 + (Math.random() * 8 | 0), 0, 0));
        }
        if ((d = this.World.Level?.IsBlocking(a, b, c)) && e < 0)
