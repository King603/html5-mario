...
export class LevelState extends Enjine.GameState {
	...
	Mario: any;
	constructor(public LevelDifficulty: number, public LevelType: number) {
		super();
		this.BgLayer = [];
		this.Paused = !1;
	...
	Enter() {
		...
		for (let a = 0; a < 2; a++) {
			...
			this.BgLayer && (this.BgLayer[a] = new BackgroundRenderer(new BackgroundGenerator(c / 32 + 1, e / 32 + 1, a === 0, this.LevelType).CreateLevel(), 320, 240, b));
		}
	...
	CheckShellCollide(a: Shell) {
		this.ShellsToCheck?.push(a);
	};
	CheckFireballCollide(a: Fireball) {
	...
	Update(Delta: number) {
		...
		this.Camera && (this.Camera.X = Mario.MarioCharacter.X - 160);
		let b;
		if (this.Camera && this.Camera.X < 0)
			this.Camera.X = 0;
		if (this.Camera && this.Level && this.Camera.X > this.Level.Width * 16 - 320)
			this.Camera.X = this.Level.Width * 16 - 320;
		for (let b = this.FireballsOnScreen = 0; this.Sprites && b < this.Sprites.Objects.length; b++) {
			let d = this.Sprites.Objects[b] as ELE;
			if (d !== Mario.MarioCharacter && this.Camera) {
		...
		if (this.Paused)
			for (b = 0; this.Sprites && b < this.Sprites.Objects.length; b++) {
				let obj = this.Sprites.Objects[b];
				obj === Mario.MarioCharacter
					? obj.Update(Delta)
					: obj.UpdateNoMove();
			}
		else {
			...
			this.Level?.Update();
			this.Tick++;
			if (this.Camera && this.Layer && this.Level)
				for (let c = (this.Camera.X / 16 | 0) - 1; c <= ((this.Camera.X + this.Layer.Width) / 16 | 0) + 1; c++)
					for (let e = (this.Camera.Y / 16 | 0) - 1; e <= ((this.Camera.Y + this.Layer.Height) / 16 | 0) + 1; e++) {
						...
						c * 16 + 8 < Mario.MarioCharacter.X - 16 && (d = 1);
						let f = this.Level.GetSpriteTemplate(c, e) as SpriteTemplate;
						if (f !== null) {
							f.LastVisibleTick !== this.Tick - 1 && (f.Sprite === null || !this.Sprites?.Contains(f.Sprite)) && f.Spawn(this, c, e, d);
							f.LastVisibleTick = this.Tick;
						}
						if (d !== 0) {
							let f = this.Level.GetBlock(c, e);
							if ((Tile.Behaviors[f & 255] & Tile.Animated) > 0 && (f % 16 / 4 | 0) === 3 && (f / 16 | 0) === 0 && (this.Tick - c * 2) % 100 === 0) {
								for (let b = 0; b < 8; b++)
									this.AddSprite(new Sparkle(this, c * 16 + 8, e * 16 + (Math.random() * 16 | 0), Math.random() * d, 0));
								this.AddSprite(new BulletBill(this, c * 16 + 8 + d * 8, e * 16 + 15, d));
							}
						}
					}
			b && Enjine.Resources.PlaySound("cannon");
			for (let b = 0; this.Sprites && b < this.Sprites.Objects.length; b++)
				this.Sprites.Objects[b].Update(Delta);
			for (let b = 0; this.Sprites && b < this.Sprites.Objects.length; b++)
				this.Sprites.Objects[b].CollideCheck();
			for (let b = 0; this.ShellsToCheck && b < this.ShellsToCheck.length; b++)
				for (let c = 0; this.Sprites && c < this.Sprites.Objects.length; c++)
					if (this.Sprites.Objects[c] !== this.ShellsToCheck[b] && !this.ShellsToCheck[b].Dead && this.Sprites.Objects[c].ShellCollideCheck(this.ShellsToCheck[b]) && Mario.MarioCharacter.Carried === this.ShellsToCheck[b] && !this.ShellsToCheck[b].Dead) {
						Mario.MarioCharacter.Carried = null as unknown as Shell;
						this.ShellsToCheck[b].Die();
					}
			if (this.ShellsToCheck)
				for (let b = this.ShellsToCheck.length = 0; this.FireballsToCheck && b < this.FireballsToCheck.length; b++)
					for (let c = 0; this.Sprites && c < this.Sprites.Objects.length; c++)
	...
	Draw(a: CanvasRenderingContext2D) {
		if (this.Camera && this.Level && this.BgLayer && this.Sprites) {
	...
	DrawStringShadow(a: CanvasRenderingContext2D, b: string, c: number, e: number) {
		this.Font && (this.Font.Strings[0] = { String: b, X: c * 8 + 4, Y: e * 8 + 4 });
		this.FontShadow && (this.FontShadow.Strings[0] = { String: b, X: c * 8 + 5, Y: e * 8 + 5 });
		this.FontShadow?.Draw(a);
		this.Font?.Draw(a);
	};
	RenderBlackout(a: CanvasRenderingContext2D, b: number, c: number, e: number) {
		if (!(e > 320)) {
			let d = [], f = [];
			for (let g = 0; g < 16; g++) {
	...
	Bump(a: number, b: number, c: boolean) {
		let e = this.Level?.GetBlock(a, b) as number;
	...
	BumpInto(a: number, b: number) {
		if ((Tile.Behaviors[(this.Level?.GetBlock(a, b) as number) & 255] & Tile.PickUpable) > 0) {
			Mario.MarioCharacter.GetCoin();
			Enjine.Resources.PlaySound("coin");
			this.Level?.SetBlock(a, b, 0);
			this.AddSprite(new CoinAnim(this, a, b + 1));
		}
