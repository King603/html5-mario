...
const MAP = Enjine.Resources.Images?.map as HTMLImageElement;
...
export class LevelRenderer extends Enjine.Drawable {
	...
	TilesY: number;
	constructor(public Level: Level, public Width: number, public Height: number) {
		super();
	...
	DrawStatic(ctx: CanvasRenderingContext2D, size: SIZE) {
	...
	DrawDynamic(ctx: CanvasRenderingContext2D, size: SIZE) {
	...
	DrawExit0(ctx: CanvasRenderingContext2D, size: SIZE, bool: boolean) {
	...
	DrawExit1(ctx: CanvasRenderingContext2D, size: SIZE) {
		for (let y = this.Level.ExitY - 8; y < this.Level.ExitY; y++) {
